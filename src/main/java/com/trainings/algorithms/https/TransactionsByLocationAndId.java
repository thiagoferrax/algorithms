...
class Result {
	...
	private static int calculateAmountSum(int locationId, int netStart, int netEnd, List<Transaction> transactions) {
		...
		for (Transaction transaction : transactions) {
			if (transaction.getLocationId().equals(locationId)
					&& isIpInNetInterval(transaction.getIp(), netStart, netEnd)) {
				if (transaction.getTxnType().equals(CREDIT)) {
					amountsSum += transaction.getAmount();
				} else {
					amountsSum -= transaction.getAmount();
				}
			}
		}
		return amountsSum.intValue();
	}
	...
	private static List<Transaction> parseJsonToTransationList(String json) {
		...
		while (matcher.find()) {
			...
			if (key.equals(AMOUNT)) {
				...
				transaction.setAmount(Double.valueOf(value.substring(1, value.length() - 1).replace(",", "")));
			}
